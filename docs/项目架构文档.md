# Go-Agent (Lattice) 项目架构文档

## 目录

1. [项目概述](#项目概述)
2. [整体架构设计](#整体架构设计)
3. [核心模块详解](#核心模块详解)
4. [模块间协作关系](#模块间协作关系)
5. [关键流程分析](#关键流程分析)
6. [技术栈与依赖](#技术栈与依赖)

---

## 项目概述

### 简介

**Go-Agent (Lattice)** 是一个用 Go 语言构建的 AI 智能代理（Agent）框架，旨在为开发者提供清晰的抽象层，简化大型语言模型（LLM）、工具调用、检索增强记忆（RAG）以及多智能体协同等功能的实现。该框架专注于生产级应用，提供可插拔的组件和模块化架构。

### 核心特性

- **🧩 模块化架构**：通过可复用模块以声明式配置构建智能体
- **🤖 多智能体支持**：通过共享目录和代理系统协调专业化智能体
- **🔧 丰富的工具系统**：一次实现 `Tool` 接口，处处自动使用
- **🧠 智能记忆系统**：基于 RAG 的记忆，支持重要性评分、MMR 检索和自动修剪
- **🔌 模型无关性**：支持 Gemini、Anthropic、Ollama 等多种 LLM 提供商
- **📡 UTCP 支持**：一流的通用工具调用协议（Universal Tool Calling Protocol）支持
- **⚡ 高性能**：通过 LRU 缓存、预分配缓冲区和并发操作优化性能

### 设计理念

1. **关注点分离**：将模型调用、记忆管理、工具编排、子智能体协同等职责清晰分离
2. **接口抽象**：通过接口定义核心组件，实现可替换性和可测试性
3. **生产就绪**：内置性能优化、错误处理、并发安全等生产级特性
4. **开发者友好**：提供清晰的 API、丰富的示例和详细的文档

---

## 整体架构设计

### 架构层次

```
┌─────────────────────────────────────────────────────────────┐
│                     应用层 (Application)                      │
│  ┌────────────┐  ┌────────────┐  ┌────────────┐            │
│  │  CLI Demo  │  │ Multi-Agent│  │  CodeMode  │  ...       │
│  └────────────┘  └────────────┘  └────────────┘            │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                    核心层 (Core Layer)                       │
│  ┌──────────────────────────────────────────────────────┐   │
│  │              Agent (智能体核心)                        │   │
│  │  • 协调模型、记忆、工具和子智能体                      │   │
│  │  • 管理对话流程和上下文                               │   │
│  │  • 支持 Checkpoint/Restore 状态持久化                │   │
│  └──────────────────────────────────────────────────────┘   │
│                                                              │
│  ┌──────────────────────────────────────────────────────┐   │
│  │         ADK (Agent Development Kit)                   │   │
│  │  • 模块化依赖注入容器                                 │   │
│  │  • 提供声明式配置方式                                 │   │
│  │  • 统一的 Provider 模式                               │   │
│  └──────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                   服务层 (Service Layer)                     │
│                                                              │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │   Models    │  │   Memory    │  │    Tools    │         │
│  │  (模型抽象)  │  │  (记忆系统)  │  │  (工具系统)  │         │
│  └─────────────┘  └─────────────┘  └─────────────┘         │
│                                                              │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │  SubAgents  │  │    Swarm    │  │    Cache    │         │
│  │ (子智能体)   │  │  (群体协同)  │  │  (缓存层)    │         │
│  └─────────────┘  └─────────────┘  └─────────────┘         │
└─────────────────────────────────────────────────────────────┘
                            ↓
┌─────────────────────────────────────────────────────────────┐
│                基础设施层 (Infrastructure)                    │
│                                                              │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐         │
│  │Vector Store │  │  Embedder   │  │Concurrent   │         │
│  │(向量存储)    │  │ (嵌入模型)   │  │ (并发工具)   │         │
│  └─────────────┘  └─────────────┘  └─────────────┘         │
│                                                              │
│  支持: PostgreSQL, Qdrant, Neo4j, MongoDB, InMemory        │
└─────────────────────────────────────────────────────────────┘
```

### 核心设计模式

1. **接口驱动设计**：
   - `models.Agent`：统一的 LLM 接口
   - `Tool`：工具的标准接口
   - `SubAgent`：子智能体接口
   - `VectorStore`：向量存储接口

2. **Provider 模式**：
   - `ModelProvider`：提供模型实例
   - `MemoryProvider`：提供记忆实例
   - `ToolProvider`：提供工具集合

3. **Module 模式**：
   - ADK 的核心模式，通过 `Module` 接口实现可插拔组件
   - 支持声明式配置和自动初始化

4. **Catalog 模式**：
   - `ToolCatalog`：管理工具注册和查找
   - `SubAgentDirectory`：管理子智能体注册和查找

---

## 核心模块详解

### 1. Agent 模块 (核心层)

**位置**: `agent.go`, `types.go`, `agent_*.go`

#### 职责

Agent 是整个框架的核心协调器，负责：
- 协调 LLM 调用
- 管理对话上下文和记忆
- 编排工具调用和子智能体委派
- 支持 UTCP 工具调用协议
- 提供状态检查点和恢复功能

#### 核心结构

```go
type Agent struct {
    model        models.Agent          // 底层 LLM 模型
    memory       *memory.SessionMemory // 会话记忆
    systemPrompt string                // 系统提示词
    contextLimit int                   // 上下文限制
    
    toolCatalog       ToolCatalog         // 工具目录
    subAgentDirectory SubAgentDirectory   // 子智能体目录
    UTCPClient        utcp.UtcpClientInterface // UTCP 客户端
    
    Shared    *memory.SharedSession      // 共享会话
    CodeMode  *codemode.CodeModeUTCP     // CodeMode 支持
    CodeChain *chain.UtcpChainClient     // 链式工具调用
}
```

#### 关键方法

1. **`New(opts Options) (*Agent, error)`**
   - 创建新的智能体实例
   - 验证必需的组件（Model 和 Memory）
   - 初始化工具目录和子智能体目录

2. **`Generate(ctx context.Context, sessionID, prompt string) (string, error)`**
   - 核心对话生成方法
   - 处理记忆检索、上下文构建、工具调用
   - 支持多轮对话和流式输出

3. **`RegisterAsUTCPProvider(ctx, client, name, desc string) error`**
   - 将 Agent 自身注册为 UTCP 工具
   - 支持智能体间通信和层级架构

4. **`Checkpoint() ([]byte, error)` / `Restore(data []byte) error`**
   - 状态持久化和恢复
   - 保存/恢复会话记忆和共享空间成员关系

#### 工作流程

```
用户输入
    ↓
检索相关记忆 (RAG)
    ↓
构建上下文 (系统提示词 + 记忆 + 用户输入 + 工具定义)
    ↓
LLM 生成响应
    ↓
解析工具调用请求 ← 需要工具?
    ↓                    ↓ 否
执行工具/子智能体      直接返回响应
    ↓
合并工具结果
    ↓
存储到记忆
    ↓
返回最终响应
```

---

### 2. ADK 模块 (Agent Development Kit)

**位置**: `src/adk/`

#### 职责

ADK 是一个轻量级的依赖注入容器和配置系统，用于：
- 模块化组装智能体组件
- 提供声明式配置方式
- 统一管理 Provider 生命周期
- 支持多种部署模式

#### 核心结构

```go
type AgentDevelopmentKit struct {
    modules      []Module              // 注册的模块列表
    bootstrapped bool                  // 是否已初始化
    
    modelProvider    ModelProvider     // 模型提供者
    memoryProvider   MemoryProvider    // 记忆提供者
    toolProviders    []ToolProvider    // 工具提供者列表
    subAgentProvider []SubAgentProvider // 子智能体提供者
    
    UTCP      utcp.UtcpClientInterface
    CodeMode  *codemode.CodeModeUTCP
    ChainMode *chain.UtcpChainClient
}
```

#### Module 接口

```go
type Module interface {
    Name() string
    Provision(ctx context.Context, kit *AgentDevelopmentKit) error
}
```

#### 内置模块

1. **ModelModule** (`src/adk/modules/model_module.go`)
   - 提供 LLM 模型实例
   - 支持懒加载和工厂模式

2. **MemoryModule** (`src/adk/modules/memory_module.go`)
   - 提供记忆系统配置
   - 支持多种向量存储后端（InMemory, Qdrant, PostgreSQL）

3. **ToolModule** (`src/adk/modules/tool_module.go`)
   - 注册工具集合
   - 支持静态工具和动态工具

4. **SubAgentModule** (`src/adk/modules/subagent_module.go`)
   - 注册子智能体
   - 支持专业化智能体组装

#### 使用示例

```go
kit, err := adk.New(ctx,
    adk.WithDefaultSystemPrompt("You are a helpful assistant."),
    adk.WithModules(
        adkmodules.NewModelModule("gemini", func(ctx context.Context) (models.Agent, error) {
            return models.NewGeminiLLM(ctx, "gemini-2.5-pro", "prefix:")
        }),
        adkmodules.InMemoryMemory(100000, memory.AutoEmbedder()),
    ),
    adk.WithSubAgents(subagents.NewResearcher(researcherModel)),
)

agent, err := kit.BuildAgent(ctx)
```

---

### 3. Memory 模块 (记忆系统)

**位置**: `src/memory/`

#### 架构分层

```
SessionMemory (会话层)
    ↓
MemoryBank (管理层)
    ↓
Engine (引擎层)
    ↓
VectorStore (存储层)
```

#### 核心组件

##### 3.1 Engine (引擎层)

**文件**: `src/memory/engine/engine.go`

**职责**：
- 记忆的评分、聚类、修剪和检索
- 语义相似度计算
- 重要性评分
- 去重和摘要生成

**关键算法**：

1. **重要性评分** (`importanceScore`)
   ```go
   // 基于内容长度、实体密度、关键词等计算重要性
   score = baseScore * lengthFactor * entityDensity * keywordBonus
   ```

2. **MMR 检索** (Maximum Marginal Relevance)
   ```go
   // 平衡相关性和多样性
   MMR(D, q) = argmax[λ * Sim(D, q) - (1-λ) * max Sim(D, d_i)]
   ```

3. **时间衰减**
   ```go
   // 根据时间降低旧记忆的权重
   decayedScore = originalScore * exp(-decay * timeDelta)
   ```

**主要方法**：
- `Store(ctx, sessionID, content, metadata)`: 存储新记忆
- `Retrieve(ctx, sessionID, query, limit)`: 检索相关记忆
- `Prune(ctx, sessionID, strategy)`: 修剪低价值记忆
- `Summarize(ctx, sessionID, records)`: 生成记忆摘要

##### 3.2 SessionMemory (会话层)

**文件**: `src/memory/session/spaces.go`

**职责**：
- 管理会话级别的记忆
- 支持短期和长期记忆
- 提供共享空间（SharedSpace）功能

**核心概念**：

1. **短期记忆**：
   - 当前会话的临时记忆
   - 使用内存存储，快速访问
   - 会话结束后可选择性持久化

2. **长期记忆**：
   - 持久化的向量存储记忆
   - 支持语义检索
   - 跨会话共享

3. **共享空间**：
   - 多个智能体可以加入同一空间
   - 在空间内共享记忆和上下文
   - 支持角色权限（Reader, Writer, Admin）

##### 3.3 VectorStore (存储层)

**接口定义**：
```go
type VectorStore interface {
    StoreMemory(ctx, sessionID, content string, metadata map[string]any, embedding []float32) error
    SearchMemory(ctx, sessionID string, embedding []float32, limit int) ([]MemoryRecord, error)
    DeleteMemory(ctx, sessionID string, ids []string) error
}
```

**支持的实现**：
- **InMemoryStore**: 内存存储，适合测试和小规模应用
- **PostgresStore**: PostgreSQL + pgvector，生产级持久化
- **QdrantStore**: 专业向量数据库，高性能检索
- **Neo4jStore**: 图数据库，支持关系图谱
- **MongoStore**: MongoDB，灵活的文档存储

##### 3.4 Embedder (嵌入层)

**文件**: `src/memory/embed/`

**支持的嵌入模型**：
- **OpenAI**: text-embedding-ada-002
- **Gemini**: text-embedding-004
- **Ollama**: 本地部署模型
- **FastEmbed**: 轻量级嵌入
- **Claude**: Anthropic 嵌入服务
- **Dummy**: 测试用随机向量

**自动选择**：
```go
embedder := memory.AutoEmbedder()
// 根据环境变量自动选择最佳嵌入模型
```

---

### 4. Models 模块 (模型抽象)

**位置**: `src/models/`

#### 统一接口

```go
type Agent interface {
    Generate(ctx context.Context, prompt string) (any, error)
    GenerateWithFiles(ctx context.Context, prompt string, files []File) (any, error)
}
```

#### 支持的提供商

##### 4.1 Gemini (Google)

**文件**: `src/models/gemini.go`

**特性**：
- 支持 Gemini 1.5/2.5 Pro 系列
- 原生多模态支持
- 工具调用（Function Calling）
- 流式输出

**使用示例**：
```go
model, err := models.NewGeminiLLM(ctx, "gemini-2.5-pro", "system prompt")
response, err := model.Generate(ctx, "What is AI?")
```

##### 4.2 Anthropic (Claude)

**文件**: `src/models/anthropics.go`

**特性**：
- Claude 3 Opus/Sonnet/Haiku 系列
- 强大的推理能力
- 长上下文支持 (200K tokens)
- 工具使用

##### 4.3 Ollama (本地部署)

**文件**: `src/models/ollama.go`

**特性**：
- 本地运行开源模型
- 无需 API 密钥
- 支持 Llama, Mistral, Phi 等模型
- 完全私有化部署

##### 4.4 OpenAI

**文件**: `src/models/openai.go`

**特性**：
- GPT-4, GPT-3.5 系列
- 工具调用
- 视觉理解
- 支持私有部署（兼容 OpenAI 协议的 API）

**私有部署配置**：

```go
// 方式1: 使用环境变量（全局配置）
export OPENAI_API_KEY="your-api-key"
export OPENAI_BASE_URL="https://your-private-deployment.com/v1"
model := models.NewOpenAILLM("gpt-4", "system prompt")

// 方式2: 直接指定 BaseURL（推荐）
model, err := models.NewOpenAILLMWithBaseURL(
    "gpt-4", 
    "system prompt",
    "https://your-private-deployment.com/v1",
)
```

##### 4.5 Cached Model

**文件**: `src/models/cached.go`

**特性**：
- 对任意模型添加缓存层
- 基于 prompt hash 缓存结果
- 减少 API 调用成本
- 提升响应速度

---

### 5. Tools 模块 (工具系统)

**位置**: `agent_tool.go`, `catalog.go`

#### 工具接口

```go
type Tool interface {
    Spec() ToolSpec
    Invoke(ctx context.Context, req ToolRequest) (ToolResponse, error)
}

type ToolSpec struct {
    Name        string           // 工具名称
    Description string           // 工具描述
    InputSchema map[string]any   // 输入参数 Schema (JSON Schema)
    Examples    []map[string]any // 使用示例
}
```

#### 工具注册与查找

```go
// 静态工具目录
catalog := agent.NewStaticToolCatalog([]agent.Tool{
    echoTool,
    calculatorTool,
})

// 注册新工具
catalog.Register(newTool)

// 查找工具
tool, spec, found := catalog.Lookup("echo")

// 获取所有工具规范（用于提示 LLM）
specs := catalog.Specs()
```

#### 工具适配器

##### SubAgentTool

将子智能体包装为工具：

```go
type SubAgentTool struct {
    subAgent SubAgent
}

// 任何 SubAgent 都可以作为 Tool 使用
tool := NewSubAgentTool(researcher)
```

##### AgentToolAdapter

将完整 Agent 包装为工具：

```go
adapter := agent.NewAgentToolAdapter(subAgent, "researcher", "Research specialist")
```

#### UTCP 集成

**UTCP (Universal Tool Calling Protocol)** 是一个标准化的工具调用协议。

**注册为 UTCP 工具**：
```go
err := agent.RegisterAsUTCPProvider(ctx, utcpClient, "local.analyst", "Analyzes data")
```

**调用 UTCP 工具**：
```go
result, err := utcpClient.CallTool(ctx, "local.analyst", map[string]any{
    "instruction": "Analyze Q4 sales",
})
```

#### CodeMode 模式

CodeMode 允许 LLM 生成 Go 代码来调用工具，实现更灵活的工具编排：

```go
kit, err := adk.New(ctx,
    adk.WithCodeModeUtcp(utcpClient, model), // 启用 CodeMode
)
```

**工作流程**：
```
用户: "分析 Q4 销售数据并生成报告"
    ↓
LLM 生成代码:
    analysis, _ := codemode.CallTool("local.analyst", map[string]any{
        "instruction": "Analyze Q4 sales",
    })
    report, _ := codemode.CallTool("local.writer", map[string]any{
        "instruction": fmt.Sprintf("Create report: %v", analysis),
    })
    __out = report
    ↓
CodeMode 执行生成的代码
    ↓
返回结果
```

---

### 6. SubAgents 模块 (子智能体)

**位置**: `src/subagents/`

#### SubAgent 接口

```go
type SubAgent interface {
    Name() string
    Description() string
    Run(ctx context.Context, input string) (string, error)
}
```

#### 内置子智能体

##### Researcher (研究员)

**文件**: `src/subagents/researcher.go`

**职责**：
- 综合背景信息
- 起草研究摘要
- 提供结构化调查结果

**实现**：
```go
type Researcher struct {
    model   models.Agent
    persona string
}

func (r *Researcher) Run(ctx context.Context, input string) (string, error) {
    prompt := r.persona + "\n\nTask:\n" + input
    resp, err := r.model.Generate(ctx, prompt)
    return fmt.Sprint(resp), err
}
```

#### 子智能体目录

```go
directory := agent.NewStaticSubAgentDirectory([]agent.SubAgent{
    researcher,
    writer,
    reviewer,
})

// 查找子智能体
subAgent, found := directory.Lookup("researcher")

// 运行子智能体
result, err := subAgent.Run(ctx, "Research quantum computing")
```

---

### 7. Swarm 模块 (群体协同)

**位置**: `src/swarm/`

#### 核心概念

Swarm 提供多智能体协同工作的框架：
- 参与者管理
- 共享记忆空间
- 角色权限控制

#### 核心结构

```go
type Swarm struct {
    *Participants
}

type Participant struct {
    ID          string
    Agent       *agent.Agent
    SharedSpace *memory.SharedSession
    JoinedSpaces []string
}
```

#### 主要功能

1. **参与者管理**：
   ```go
   swarm := swarm.NewSwarm(participants)
   participant := swarm.GetParticipant("agent-1")
   ```

2. **空间管理**：
   ```go
   // 加入共享空间
   swarm.Join("agent-1", "project-alpha")
   
   // 离开空间
   swarm.Leave("agent-1", "project-alpha")
   ```

3. **记忆同步**：
   ```go
   // 保存所有参与者记忆
   swarm.Save(ctx)
   
   // 检索参与者记忆
   memories, err := swarm.Retrieve(ctx, "agent-1")
   ```

---

### 8. Cache 模块 (缓存层)

**位置**: `src/cache/`

#### LRU Cache

**文件**: `src/cache/lru_cache.go`

**特性**：
- 固定容量的 LRU (Least Recently Used) 缓存
- 线程安全
- O(1) 访问和更新
- 支持泛型

**实现**：
```go
type LRUCache[K comparable, V any] struct {
    capacity int
    items    map[K]*list.Element
    order    *list.List
    mu       sync.RWMutex
}
```

**使用场景**：
- MIME 类型规范化缓存 (10-50x 性能提升)
- 工具规范缓存
- 嵌入向量缓存
- 提示词模板缓存

**使用示例**：
```go
cache := cache.NewLRUCache[string, string](1000)
// cache.NewLRUCache[string, string](1000) 这个是什么语法? 这是 Go 1.18 引入的泛型语法，表示创建一个键类型为 string，值类型为 string 的 LRUCache 实例，容量为 1000。
cache.Put("key", "value")
value, found := cache.Get("key")
```

---

### 9. Concurrent 模块 (并发工具)

**位置**: `src/concurrent/`

#### Worker Pool

**文件**: `src/concurrent/pool.go`

**特性**：
- 固定大小的工作池
- 并发任务执行
- 错误聚合
- 优雅关闭

**使用场景**：
- 批量嵌入生成
- 并行工具调用
- 批量记忆检索

---

## 模块间协作关系

### 依赖关系图

```
┌─────────────────────────────────────────────────────────┐
│                      Application                         │
└───────────────────────┬─────────────────────────────────┘
                        │
                        ↓
┌─────────────────────────────────────────────────────────┐
│                    Agent (Core)                          │
│  依赖: Models, Memory, Tools, SubAgents, UTCP          │
└───┬────────┬────────┬────────┬────────┬────────────────┘
    │        │        │        │        │
    ↓        ↓        ↓        ↓        ↓
┌──────┐ ┌──────┐ ┌──────┐ ┌────────┐ ┌──────┐
│Models│ │Memory│ │Tools │ │SubAgent│ │ UTCP │
└──┬───┘ └──┬───┘ └──────┘ └───┬────┘ └──────┘
   │        │                   │
   │        ↓                   │
   │   ┌────────────┐          │
   │   │   Engine   │          │
   │   └─────┬──────┘          │
   │         │                  │
   │         ↓                  │
   │   ┌────────────┐          │
   │   │VectorStore │          │
   │   └─────┬──────┘          │
   │         │                  │
   │         ↓                  │
   │   ┌────────────┐          │
   │   │  Embedder  │←─────────┘
   └───┤            │
       └────────────┘
```

### 交互流程

#### 场景 1: 用户查询处理

```
用户
 │
 ├──→ Agent.Generate()
      │
      ├──→ Memory.Retrieve()  (检索相关记忆)
      │    │
      │    └──→ Engine.Retrieve()
      │         │
      │         └──→ VectorStore.SearchMemory()
      │
      ├──→ Models.Generate()  (LLM 生成响应)
      │
      ├──→ 解析工具调用
      │
      ├──→ ToolCatalog.Lookup()  (查找工具)
      │
      ├──→ Tool.Invoke()  (执行工具)
      │
      ├──→ Memory.Store()  (存储新记忆)
      │    │
      │    └──→ Engine.Store()
      │         │
      │         └──→ VectorStore.StoreMemory()
      │
      └──→ 返回响应
```

#### 场景 2: 多智能体协同

```
主 Agent
 │
 ├──→ SubAgentDirectory.Lookup("researcher")
 │    │
 │    └──→ 返回 Researcher SubAgent
 │
 ├──→ SubAgent.Run()
 │    │
 │    └──→ Models.Generate()
 │
 ├──→ 整合结果
 │
 └──→ Memory.Store()  (存储到共享空间)
      │
      └──→ 其他 Agent 可访问
```

#### 场景 3: CodeMode 工具编排

```
用户输入
 │
 ├──→ CodeMode Agent
 │    │
 │    ├──→ Models.Generate()  (生成 Go 代码)
 │    │
 │    └──→ CodeMode.Execute()
 │         │
 │         ├──→ UTCPClient.CallTool("tool1")
 │         │    │
 │         │    └──→ Agent1.Generate()
 │         │
 │         ├──→ UTCPClient.CallTool("tool2")
 │         │    │
 │         │    └──→ Agent2.Generate()
 │         │
 │         └──→ 返回聚合结果
 │
 └──→ 返回最终输出
```

---

## 关键流程分析

### 流程 1: Agent 初始化

```go
// 1. 创建依赖组件
model, _ := models.NewGeminiLLM(ctx, "gemini-2.5-pro", "You are helpful")
store := memory.NewInMemoryStore()
engine := memory.NewEngine(store, memory.DefaultOptions())
memoryBank := memory.NewMemoryBankWithStore(store, engine)
sessionMemory := memory.NewSessionMemory(memoryBank)

// 2. 准备工具和子智能体
tools := []agent.Tool{echoTool, calcTool}
subAgents := []agent.SubAgent{researcher}

// 3. 创建 Agent
agent, err := agent.New(agent.Options{
    Model:        model,
    Memory:       sessionMemory,
    SystemPrompt: "You are a helpful assistant",
    ContextLimit: 10,
    Tools:        tools,
    SubAgents:    subAgents,
})

// 4. (可选) 注册为 UTCP 工具
utcpClient := utcp.NewClient()
agent.RegisterAsUTCPProvider(ctx, utcpClient, "local.assistant", "AI assistant")
```

### 流程 2: 对话生成详解

```go
response, err := agent.Generate(ctx, "session-123", "计算 42 * 17")
```

**内部步骤**：

1. **记忆检索** (RAG):
   ```go
   // 嵌入用户查询
   queryEmbedding := embedder.Embed(ctx, "计算 42 * 17")
   
   // 检索相关记忆
   memories := engine.Retrieve(ctx, "session-123", queryEmbedding, 5)
   // 返回: [
   //   {content: "用户之前问过乘法", similarity: 0.85},
   //   {content: "用户喜欢数学", similarity: 0.72},
   // ]
   ```

2. **构建上下文**:
   ```go
   prompt := buildPrompt(
       systemPrompt,    // "You are a helpful assistant"
       memories,        // 相关记忆
       userQuery,       // "计算 42 * 17"
       toolSpecs,       // 可用工具列表
   )
   // 结果示例:
   // """
   // System: You are a helpful assistant
   // 
   // Memory:
   // - User previously asked about multiplication
   // 
   // Available Tools:
   // - calculator: Performs basic math operations
   // 
   // User: 计算 42 * 17
   // """
   ```

3. **LLM 生成**:
   ```go
   response := model.Generate(ctx, prompt)
   // LLM 可能返回:
   // "I'll use the calculator tool: 
   //  {"tool": "calculator", "arguments": {"a": 42, "b": 17, "op": "multiply"}}"
   ```

4. **解析工具调用**:
   ```go
   toolCalls := parseToolCalls(response)
   // [
   //   {name: "calculator", args: {a: 42, b: 17, op: "multiply"}}
   // ]
   ```

5. **执行工具**:
   ```go
   tool, spec, _ := toolCatalog.Lookup("calculator")
   result, _ := tool.Invoke(ctx, ToolRequest{
       SessionID: "session-123",
       Arguments: {a: 42, b: 17, op: "multiply"},
   })
   // result.Content = "714"
   ```

6. **合并结果**:
   ```go
   finalPrompt := prompt + "\n\nTool result: 714\n\nProvide final answer:"
   finalResponse := model.Generate(ctx, finalPrompt)
   // "42 * 17 = 714"
   ```

7. **存储记忆**:
   ```go
   memory.Store(ctx, "session-123", 
       "User asked: 计算 42 * 17. Answer: 714",
       map[string]any{"importance": 0.6})
   ```

### 流程 3: 记忆检索与评分

```go
memories, err := engine.Retrieve(ctx, "session-123", queryEmbedding, 10)
```

**详细步骤**：

1. **向量检索**:
   ```go
   candidates := vectorStore.SearchMemory(ctx, "session-123", queryEmbedding, 50)
   // 返回前 50 个语义相似的记忆
   ```

2. **综合评分**:
   ```go
   for each candidate:
       // 语义相似度 (0-1)
       semanticScore := cosineSimilarity(queryEmbedding, candidate.Embedding)
       
       // 时间衰减
       age := now - candidate.CreatedAt
       timeScore := exp(-decayRate * age)
       
       // 重要性
       importanceScore := candidate.Importance
       
       // 综合评分
       finalScore := 
           weights.Semantic * semanticScore +
           weights.Temporal * timeScore +
           weights.Importance * importanceScore
   ```

3. **MMR 去重**:
   ```go
   // Maximum Marginal Relevance
   selected := []Memory{}
   while len(selected) < limit:
       best := argmax(
           λ * relevance(m) - (1-λ) * max_similarity(m, selected)
       )
       selected.append(best)
   ```

4. **返回结果**:
   ```go
   return selected.sortBy(score).limit(10)
   ```

### 流程 4: 子智能体委派

```go
// 主 Agent 委派给 Researcher
result, err := agent.Generate(ctx, "session-123", 
    "Research the latest trends in quantum computing")
```

**处理流程**：

1. **识别委派意图**:
   ```go
   // LLM 分析用户请求
   if needsSubAgent(query):
       subAgentName = selectSubAgent(query, subAgentDirectory.All())
       // 返回: "researcher"
   ```

2. **查找子智能体**:
   ```go
   subAgent, found := subAgentDirectory.Lookup("researcher")
   ```

3. **委派执行**:
   ```go
   result, err := subAgent.Run(ctx, 
       "Research the latest trends in quantum computing")
   // Researcher 内部:
   // 1. 构建专业提示词
   // 2. 调用自己的 LLM
   // 3. 返回结构化结果
   ```

4. **整合结果**:
   ```go
   finalResponse := integrateSubAgentResult(
       originalQuery,
       subAgentResult,
       mainAgentContext,
   )
   ```

### 流程 5: Checkpoint 与 Restore

```go
// 保存状态
data, err := agent.Checkpoint()
os.WriteFile("agent-state.json", data, 0644)

// 恢复状态
data, _ := os.ReadFile("agent-state.json")
newAgent, _ := agent.New(opts)
newAgent.Restore(data)
```

**Checkpoint 包含**：
```json
{
  "system_prompt": "You are a helpful assistant",
  "short_term": {
    "session-123": [
      {
        "content": "Previous conversation...",
        "embedding": [0.1, 0.2, ...],
        "importance": 0.8,
        "created_at": "2026-02-14T10:00:00Z"
      }
    ]
  },
  "joined_spaces": ["project-alpha", "team-beta"],
  "timestamp": "2026-02-14T12:00:00Z"
}
```

**应用场景**：
- 跨进程/服务器迁移
- 热重启保持状态
- 版本回滚
- 调试和审计

---

## 技术栈与依赖

### 核心依赖

```
Go 1.25+
│
├── LLM SDKs
│   ├── google/generative-ai-go (Gemini)
│   ├── anthropics/anthropic-sdk-go (Claude)
│   ├── ollama/ollama (本地模型)
│   └── sashabaranov/go-openai (OpenAI)
│
├── 向量存储
│   ├── jackc/pgx/v5 (PostgreSQL)
│   ├── qdrant (Qdrant 客户端)
│   ├── neo4j/neo4j-go-driver/v5 (Neo4j)
│   └── go.mongodb.org/mongo-driver (MongoDB)
│
├── 嵌入模型
│   ├── anush008/fastembed-go (FastEmbed)
│   └── (集成各 LLM SDK 的嵌入 API)
│
├── UTCP
│   └── universal-tool-calling-protocol/go-utcp
│
└── 工具库
    ├── alpkeskin/gotoon (ASCII 动画)
    └── google/uuid (UUID 生成)
```

### 性能优化技术

1. **LRU 缓存**: 184 ns/op 缓存操作
2. **预分配缓冲区**: 减少 40-60% 内存分配
3. **并发工作池**: 并行处理批量任务
4. **MIME 规范化缓存**: 10-50x 性能提升
5. **字符串操作优化**: 减少 30-50% 开销

### 环境变量配置

| 变量 | 说明 | 必需 |
|------|------|------|
| `GOOGLE_API_KEY` | Gemini API 密钥 | 使用 Gemini 时 |
| `ANTHROPIC_API_KEY` | Claude API 密钥 | 使用 Claude 时 |
| `OPENAI_API_KEY` | OpenAI API 密钥 | 使用 OpenAI 时 |
| `OPENAI_BASE_URL` | 自定义 OpenAI API 地址（用于私有部署） | 否 |
| `DATABASE_URL` | PostgreSQL 连接字符串 | 使用 PostgreSQL 存储时 |
| `QDRANT_URL` | Qdrant 服务地址 | 使用 Qdrant 时 |
| `ADK_EMBED_PROVIDER` | 嵌入提供商 (gemini/openai/ollama) | 否 |
| `NEO4J_URI` | Neo4j 连接 URI | 使用 Neo4j 时 |
| `MONGODB_URI` | MongoDB 连接 URI | 使用 MongoDB 时 |

### 项目特色

1. **生产级设计**:
   - 完善的错误处理
   - 并发安全
   - 资源管理（context 取消、超时）
   - 性能基准测试

2. **可测试性**:
   - 接口驱动设计
   - Mock 友好
   - 单元测试覆盖
   - 集成测试示例

3. **扩展性**:
   - 插件式模块系统
   - 自定义 Provider
   - 可替换组件

4. **开发者体验**:
   - 清晰的 API
   - 丰富的示例 (cmd/example/)
   - 详细的注释
   - 类型安全

---

## 总结

Go-Agent (Lattice) 是一个设计精良、功能完善的 AI 智能体框架，具有以下优势：

✅ **模块化与可组合性**：通过 ADK 和 Module 系统实现灵活的组件组装  
✅ **生产就绪**：内置性能优化、错误处理、并发安全  
✅ **多模型支持**：统一接口适配多种 LLM 提供商  
✅ **智能记忆系统**：基于 RAG 的语义检索和智能评分  
✅ **丰富的工具生态**：UTCP 协议支持和 CodeMode 编排  
✅ **多智能体协同**：SubAgent 和 Swarm 支持复杂工作流  

该框架适合构建从简单聊天机器人到复杂多智能体系统的各类 AI 应用。
